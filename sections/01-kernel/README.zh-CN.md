...
> 这套 harness 里，东西到 runtime 才挂上，也在 runtime 拆掉。收尾只靠约定，哪个 plugin 忘了就拆不干净，所以你注册时就顺手把该怎么复原交给框架。
...
约定会走样。某个 plugin 在新的代码路径上加了一个监听器，却忘了补上对应的取消，于是卸载它之后，就漏了一个回调函数，永远对着一个已经死掉的 plugin 触发。
