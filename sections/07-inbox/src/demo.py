"""Live demo: section 07 steering against the real Anthropic API.

The loop and scheduler do not change: the demo exercises the three
inbox routings. inject() parks context while the agent is idle; the
first send() claims it alongside the prompt. Mid-turn, a bus listener
steers when the first tool result lands, and queues a followup: the
steering joins the running turn at its next step boundary, while the
followup gets a turn of its own after this one closes. This file is
the only place the SDK and the mini-Message to Anthropic translation
live (ADR 0001). Scripted turns only; skips politely when the key or
the live-demo deps are missing.
"""
...
def main():
    ...
    # the loop appends shows the moment it lands in the log. The one-shot
    # steer plus followup fire when the first tool result lands, mid-turn.
    steered = [False]
    ...
    def on_event(_session, event):
        ...
        if event["type"] == "assistant/chunk":
            ...
            if not steered[0]:
                steered[0] = True
                print("  [steer, mid-turn: joins this turn at the next step boundary]")
                agent.steer("While you are at it, also read my coffee note.")
                print("  [followup, mid-turn: queued for a turn of its own]")
                agent.followup("Now say goodbye in one short line.")
    ...
    print("[inject, while idle: context parks in the inbox, no turn starts]")
    agent.inject("Context for your answers: the user's name is Marcus.")

    # One send, three routings visible: the injected context rides in with
    # this prompt, the steer spends an extra step inside the first turn, and
    # the followup runs as a second turn before send() returns.
    agent.send("What is my wifi password? Use the lookup tool, and greet me by name.")
