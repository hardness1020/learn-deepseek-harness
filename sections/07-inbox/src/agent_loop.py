...
registry, and an inbox of pending input, nothing else worth saving. A
turn is a while-step loop; each step claims the inbox, re-derives model
history from the log, streams one model call through the seam, appends
everything back, and runs the reply's tool calls through the pipeline.
...
class Agent:
    ...
    def _step(self, target):
        """One step: claim the inbox, re-derive history, one model call, tools."""
