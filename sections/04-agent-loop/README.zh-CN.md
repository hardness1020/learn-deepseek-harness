...
5. Agent 这个对象上不留任何持久的东西：任何一个 Agent 只要接到同一份 log，都能接得一模一样，所以接续就等于把 log 重播一遍，再配一个新的 Agent。
...
接续的时候也划算。Agent 身上就只有一个 session、一个 Model seam 的 callable，还有一个 `status` 标志，而那个标志只表示“现在正在一个 turn 中间”。把 log 重播进一个新的 session，交给一个全新的 Agent，接下来那个 turn 写进去的每一行，会跟原本那个 Agent 会写的一模一样。
...
真正的 agent loop 在这个 section 的 Mechanism 之上多做了什么：
...
- **缓存一份消息列表，等于把真相抄了第二份。** 历史一旦存在一份活的列表里，其他每个 Mechanism 都会变成同步问题：compaction 在它背后改 surface，replay 重建 session 的时候根本不会理它。每个 step 都从 log 推导，就代表从头到尾没有东西需要同步。
...
- **少了边界标记，replay 就分不清楚了。** 没有 `turn/start` 和 `step/end` 这两行，重播的人分不出来一个 turn 是好好结束的，还是跑到一半崩掉的。这些边界是数据，不是随手打印出来 debug 用的东西：有它们，log 才是一个故事，而不是一堆散掉的消息。
...
- [`test.py`](src/test.py)：整个 turn 的故事会照顺序落在 log 上；`request/header` 上的数字证明每一步都重新推导，跨过一次 compaction 也一样（1、3、2）；把 log 重播一遍再配一个新的 Agent，接下去写的东西一模一样；step 中途崩掉，下一次推导还是干净的；turn 中途再调用一次 `send()` 会被拒绝。
