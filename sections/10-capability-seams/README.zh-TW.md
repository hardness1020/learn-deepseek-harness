...
但這個位置會把三個決定焊進同一個函式裡：model 看見什麼、契約是什麼、由哪台機器做事。Offline 檢查需要把檔案放在記憶體裡；你的筆電需要真正的磁碟；鎖得很緊的主機兩者都不需要。要用同一個 tool 本體服務這三種情境，你就會把那個本體重寫三遍，而且每重寫一次，model 拿來規劃的 schema 都會跟著變。
...
- **真的關得住。** `sandbox-local` 會串起各平台的執行器：linux 上是 `bwrap` 和 `landlock`，darwin 上是 `seatbelt`（第 160 行），還有一個 Windows ACL 的 Provider。那一整套機器就是 Mini-dsh 的 Ceiling：只會改寫 argv 的替身留住了 seam 的形狀，也留住了出事就關死的規則，但它其實什麼都擋不住；真正的隔離只在這裡指給你看，沒有做。
