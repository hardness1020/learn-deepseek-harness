...
> 把能力写进 tool 本体，model 看到的样子、契约和机器就绑成一块。一开始就把三者都拆开，只会多出一堆只有一种实现、根本没人会替换的抽象层。所以只有当某段代码不能知道是哪台机器回答时，才拆。

走到第十个 Section，mini-dsh 还是碰不到自己那份 log 以外的任何东西。第一个真正的能力，不管是读一个文件还是跑一条命令，总得住在某个地方，而最顺手的地方就是 tool 的本体。

但这个位置会把三个决定焊进同一个函数里：model 看见什么、契约是什么、由哪台机器做事。Offline 检查需要把文件放在内存里；你的笔记本需要真正的磁盘；锁得很紧的主机两者都不需要。要用同一个 tool 本体服务这三种情境，你就会把那个本体重写三遍，而且每重写一次，model 拿来规划的 schema 都会跟着变。
...
- **真的关得住。** `sandbox-local` 会串起各平台的运行器：linux 上是 `bwrap` 和 `landlock`，darwin 上是 `seatbelt`（第 160 行），还有一个 Windows ACL 的 Provider。那一整套机器就是 Mini-dsh 的 Ceiling：只会改写 argv 的替身留住了 seam 的形状，也留住了出事就关死的规则，但它其实什么都挡不住；真正的隔离只在这里指给你看，没有做。
