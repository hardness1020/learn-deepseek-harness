...
> 一份清單是菜單，不是那頓飯。每一次 request 都要為那些名字付錢，一個名字一行；全文要等 model 開口點了才會上桌。
...
而且這組東西不是固定的。skill 文字的主人很多：內建的一批、一個工作區、一個 plugin。session 還在跑的時候，每一方都可能掛上、卸下，或者蓋掉某個名字，而且誰都不准為了這件事去改別人的文字。
